import type { Music } from "~/types";
import { getPerson } from "../../persons";

export const bourei_in_the_water: Music = {
  type: "music",
  id: "bourei_in_the_water",
  title: "bourei in the water",
  description: `
吉田音楽製作所 合作コンピ2022参加曲です。
編曲で参加しました。
Myca. さんによる再録はこちら: https://www.nicovideo.jp/watch/sm45332566
`,
  tags: ["合作参加作品", "バンドサウンド", "夏"],
  credits: [
    {
      name: "Myca.",
      role: "作詞、作曲",
      socialLinks: [
        {
          platform: "niconico",
          url: "https://www.nicovideo.jp/user/121767144",
        },
        {
          platform: "youtube",
          url: "https://www.youtube.com/@myca.940",
        },
        {
          platform: "twitter",
          url: "https://x.com/saaax85",
        },
      ],
    },
    {
      ...getPerson("竹馬あお"),
      role: "編曲",
    },
    {
      name: "Tormanerico",
      role: "編曲",
      socialLinks: [
        {
          platform: "twitter",
          url: "https://x.com/Mawaribe199932",
        },
      ],
    },
  ],
  team: "がき、竹馬あお、周辺",
  releaseDate: new Date("2022-10-23"),
  links: [
    {
      platform: "bandcamp",
      url: "https://kitchon.bandcamp.com/track/bourei-in-the-water-2",
    },
  ],
  lyrics: `
浅瀬に立つ　手を振って
君の背中を見送って
内側から壊れてく
内側から消えてゆく

ああ

不安を高く蹴り上げて
透かしてみたら綺麗だね
ゆるい流れに逆らって
日差しで死んで笑っている

死んだように生きている
魚たちとすれ違う
亡霊に似た僕たちは
ずるいかもね　
笑って　ほら

きっと失くしてしまうだろう
入道雲　過ぎていく
朽ちてくよ　それでもいい
ぼくは置き去りにしたくない
だから

月の真下　三角形
火花で夢を撃ち落とす
君が翔んだ水面は
何もないから美しい

水平線のその先
誰も知らない未来の音
嘘みたいな朝焼けの前で
怯えていた

なぜ、なぜ、なぜ、なぜ？
誰、誰、誰、誰？

……

「この世は間違いだらけだね」

……

きっと失くしてしまうだろう
手を離せば　微熱みたいに
朽ちてくよ　それでもいい
ぼくは置き去りに
されたくなかったんだ

助けてよ

雨、降ってるね
そんなことないのに嘘をついた
いま　何もかも見えるから
遠くへふりむかずに
雨降ってるね　降ってるね
それなら亡霊になって
いま何もかも、助けるよ、助かるよ
助けて
`,
};
